 ...
 */
#define MAXIMO_CHAVES 510 /* 2n = 510; n = 255 */
...
typedef struct No {
    unsigned int byte_offset; /* tbm é a chave/ID */
} TNo;

/* RepresentaÁ„o de p·gina */
typedef struct Pagina { 
    unsigned short num_chaves; /* n˙mero de chaves */
    TNo entradas[MAXIMO_CHAVES]; /* chaves */
    int ponteiros[MAXIMO_CHAVES+1];  /* ponteiros indicando a p·gina */
    char vazio[10]; /* Usado para preencher a p·gina de 1024 bytes */
} Pagina;
...
int pagina_inserir(Arvore *arv, Pagina *pag, TNo elem);
...
int arvore_inserir(Arvore *arv, TNo elem);
...
int pagina_split(Arvore *arv, Pagina *pag, int pagina, int pai) {
    ...
    if(pai >= 0) {
        ...
        memset(pag_pai.entradas, 0, MAXIMO_CHAVES*sizeof(TNo));
        NULLIFICAR(pag_pai.ponteiros);
    }

    /* Cria a nova p·gina */
    pag_nova.num_chaves = 0;
    memset(pag_nova.entradas, 0, MAXIMO_CHAVES*sizeof(TNo));
...
void arvore_iniciar(Arvore *arv) {
    ...
    memset(pag.entradas, 0, MAXIMO_CHAVES*sizeof(TNo));
